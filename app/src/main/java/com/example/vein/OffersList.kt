...
class OffersList : AppCompatActivity() {
    ...
    private fun downloadData(message1:String,message2:String){
        ...
        database.child("DonationOffers").child(message1).child(message2).addValueEventListener(object : ValueEventListener {
    ...
    private fun downloadAllData(){
        ...
        database.child("DonationAllOffers").addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                medicalOrderList.clear()
                for (postSnapshot in snapshot.children){
                    val currentDisease = postSnapshot.getValue(DonationOffer::class.java)
